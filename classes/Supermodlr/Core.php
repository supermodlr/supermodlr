...
abstract class Supermodlr_Core {
   ...
   //bind a model as the owner of this object
   public function bind_user(Supermodlr $User)
   {
        $this->cfg('bound_user',$User);
   }


   public function get_user_access_tags()
   {
        $User = $this->cfg('bound_user');
        if ($this->cfg('bound_user') !== NULL && $this->cfg('bound_user') InstanceOf Model_Supermodlruser && isset($User->useraccesstags)) 
        {
            return $User->useraccesstags;
        }
        return $this->cfg('user_access_tags');
   }

   public function get_model_access_rules()
   {
    $access = $this->cfg('model_access_rules');
    $params['access_rules'] = &$access;
    $this->model_event('get_model_access_rules',$params);
    return $access;
   }

        ...
        if (isset($params['fields']) && isset($params['model']))
        {
            foreach ($params['fields'] as $field_key => $Field)
            {
                if (isset($params['set'][$field_key]))
                {
                    ...
                    {
                        if (method_exists($this, $Field->datatype.'_todb'))
                        {
                    ...
                    else if ($Field->storage == 'array' && is_array($params['set'][$field_key]))
                    {
                        foreach ($params['set'][$field_key] as $i => $field_value)
                        {
                            if (method_exists($this, $Field->datatype.'_todb'))
                            {
                                $method = $Field->datatype.'_todb';
                                $this->$method(array(
                                    'value'=> &$params['set'][$field_key][$i],
                                    'set'  => &$params['set'],
                                    'field'=> $Field,
                                    'model'=> $params['model'],
                                ));

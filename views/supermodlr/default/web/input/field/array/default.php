...
{
	//
?><select ui-select2 class='input input-medium' id="<?=$form_id; ?>__field__<?php echo $field->name; ?>" 
name="field__<?php echo $field->name; ?>" ng-model="data.<?php echo $field->model_name; ?>.<?php echo $field->name; ?>" 
ng-init="<?php echo $field->model_name; ?>.<?php echo $field->name; ?>='<?php 
if ($field->value_isset()) 
	...
	foreach ($field->values as $k => $option_value)
	{
		...
		if (($field->raw_value == $option_value) || (!$field->value_isset() && $option_value === $field->defaultvalue)) 
		{ 
...
<input type="hidden" ng-change="validate('<?=$field->path('_'); ?>')" ng-model="data.<?php echo $field->model_name; ?>.<?php echo $field->name; ?>" 
...
if ($field->nullvalue !== FALSE || $field->defaultvalue !== NULL) { 
	?>ng-init="<?=$field->model_name; ?>.<?=$field->path('.'); ?>='<?php if ($field->value_isset()) { echo $field->value; } else if ($field->defaultvalue) { echo $field->defaultvalue; }?>'"<?php
...
function <?=$form_id ?>__<?=$field->path('_'); ?>__add(name) {
	...
	if (typeof scope.data.<?=$field->model_name; ?>.<?=$field->path('.'); ?> == 'undefined' || scope.data.<?=$field->model_name; ?>.<?=$field->path('.'); ?> == null) {
	...
	//if the scope already has a value
		...
		var arr = scope.data.<?=$field->model_name; ?>.<?=$field->path('.'); ?>;
	...
	if (!exists) {
		...
		scope.data.<?=$field->model_name; ?>.<?=$field->path('.'); ?> = arr;
...
function <?=$form_id ?>__<?=$field->path('_'); ?>__remove(name) {
	...
	//set the object as the model.fields value
	scope.data.<?=$field->model_name; ?>.<?=$field->path('.'); ?> = new_arr;	
}

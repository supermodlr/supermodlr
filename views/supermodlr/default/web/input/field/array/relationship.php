<input class='input' name="<?=$field->model_name ?>__<?=$field->path('_') ?>__autocomplete" id="<?=$form_id; ?>__<?=$field->path('_') ?>__autocomplete" type="text"/>
<input class='input' type="text" id="<?=$form_id; ?>__field__<?=$field->path('_') ?>" name="<?=$field->model_name ?>__<?=$field->path('_') ?>" ng-model="data.<?=$field->model_name ?>.<?=$field->path('.') ?>" autocomplete="off" style="display: none"/>
...
$("#<?=$form_id; ?>__<?=$field->path('_') ?>__autocomplete").autocomplete({
            source: function(request,response) { /*@todo  support field->source as an array to search multiple models and convert $regex to a standard "like" syntax */
				var url = getAPIPath()+'/<?=$field->model_name ?>/relsearch/<?=$field->path('.') ?>?q='+request.term;
...
function <?=$form_id; ?>__<?=$field->path('_') ?>__add(obj,label) {
	...
	//if the scope already has a value
		...
		var arry = scope.data.<?=$field->model_name ?>.<?=$field->path('.') ?>;
	...
	if (!exists) {
		...
		scope.data.<?=$field->model_name ?>.<?=$field->path('.') ?> = arry;
	}
...
function <?=$form_id; ?>__<?=$field->path('_') ?>__remove(obj_hash) {
	...
	//set the object as the model.fields value
	scope.data.<?=$field->model_name ?>.<?=$field->path('.') ?> = new_arry;	

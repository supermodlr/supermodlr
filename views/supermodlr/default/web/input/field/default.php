...
?><select ui-select2 class='input input-medium' id="<?=$form_id; ?>__field__<?=$field->path('_'); ?>" name="field__<?=$field->path('_'); ?>" 
		ng-model="data.<?=$field->model_name; ?>.<?=$field->path('.'); ?>" 
		<?php if ($field->nullvalue !== FALSE || $field->value_isset() || $field->defaultvalue) 
		{ ?>ng-init='data.<?=$field->model_name; ?>.<?=$field->path('.'); ?>=<?php 
...
?>" ng-model="data.<?=$field->model_name; ?>.<?=$field->path('.'); ?>" id="<?=$form_id; ?>__field__<?=$field->path('_'); ?>" name="field__<?=$field->path('_'); ?>" value="<?php 
if ($field->value_isset()) 
{
		echo $field->value; 
}?>" <?php

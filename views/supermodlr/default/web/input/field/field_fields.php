...
<input class='input' type="text" id="<?=$form_id; ?>__field__fields" name="field__fields" ng-model="data.field.fields" autocomplete="off" style="display: none" json="true"/>
...
$("#<?=$form_id; ?>__field__fields__autocomplete").autocomplete({
            source: function(request,response) {
				...
				$.getJSON( url, request, function( server_data, status, xhr ) {
					...
					var arr = scope.data.<?=$field->model_name ?>.fields;
...
function <?=$form_id; ?>__fields__showdetails(field,action) {
	...
	if (typeof scope.data.field._id == 'undefined' || !scope.data.field._id)
	{
		$('#<?=$form_id; ?>__field__fields__add_form').html('You must save a parent field before sub-fields can be added.');
	...
	var id = scope.data.field._id;

	//preloaded form with model already selected
	var data = {"parentfield":{"model":"field","_id":id}};

	if (typeof scope.data.field.model != "undefined" && scope.data.field.model) {
		data.model = {"model": "model", "_id": scope.data.field.model};
	}
	...
	$.ajax({
		'url': '/supermodlr/api/field/form/'+field._id+'/'+action+'?data='+JSON.stringify(data),
...
function <?=$form_id; ?>__fields__add(obj,label) {
	...
	if (typeof scope.data.field.fields == 'undefined' || scope.data.field.fields == null) {
	...
	//if the scope already has a value
		...
		var arr = scope.data.field.fields;
		for (var fi = 0; fi < arr.length; fi++) {
...
function <?=$form_id; ?>__fields__remove(obj_id) {
	...
	scope.data.<?=$field->model_name ?>.fields = new_arr;	
...
$("#<?=$form_id; ?>__field__fields__add_container").dialog({
    ...
    buttons: {
        "Add Field": function() {
			...
			if (typeof scope.data.<?=$field->model_name ?>._id == 'undefined' || !scope.data.<?=$field->model_name ?>._id)
			{ 

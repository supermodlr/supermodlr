...
<input class='input' type="text" id="<?=$form_id; ?>__field__<?=$field->path('_') ?>" name="model__<?=$field->path('_') ?>" ng-model="data.<?=$field->get_model_name() ?>.<?=$field->path('.') ?>" autocomplete="off" style="display: none" json="true"/>
...
$("#<?=$form_id; ?>__field__<?=$field->path('_') ?>__autocomplete").autocomplete({
            source: function(request,response) {
                ...
                var url = getAPIPath()+'/field/query/?q={"where":{"name":{"\$regex":"/^'+request.term+'.*/i"}}}';
...
function <?=$form_id; ?>__<?=$field->path('_') ?>__showdetails(field,action) {
    ...
    var data = {"model":{"model":"<?=$field->get_model_name() ?>","_id":id}};
